...
try {
  ...
  assert.equal(await optionsPage.getByRole("button", { name: "Create Account First" }).count(), 2);
  await optionsPage.getByRole("button", { name: "Create Account First" }).nth(1).click();
  await optionsPage.waitForFunction(() => document.activeElement?.id === "options-signup-name");
...
`;
  ...
  await unauthPopupPage.waitForFunction(() => /Enter your name/i.test(document.querySelector("#popup-auth-message")?.textContent || ""));
  ...
  await unauthPopupPage.locator("#popup-auth-name").fill("Test User");
  await unauthPopupPage.locator("#popup-send-otp").click();
