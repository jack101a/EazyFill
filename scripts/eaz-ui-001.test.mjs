...
`;
  ...
  assert.equal(await unauthPopupPage.locator("#account-link-copy").innerText(), "Sign in / Sign up");
  await unauthPopupPage.locator("#account-link").click();
  await unauthPopupPage.waitForSelector("#popup-auth-panel:not([hidden])");
  assert.equal(await unauthPopupPage.locator("#popup-auth-title").innerText(), "Sign in / Sign up");
